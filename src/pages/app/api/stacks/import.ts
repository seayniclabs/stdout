...
export const POST: APIRoute = async ({ locals, request }) => {
  ...
  try {
    const { createMonitorsFromScan, executeMonitorCreation } = await import('../../../../lib/observatory/auto-monitor');

    // Get or create stack for monitors
    let stack = db.select().from(schema.stacks)
      .where(eq(schema.stacks.userId, locals.user.id))
    ...
    if (!stack) {
      ...
      const now = new Date();
      db.insert(schema.stacks).values({
        id: stackId,
        userId: locals.user.id,
        name: 'My Infrastructure',
        description: 'Auto-generated by Observatory AI',
        createdAt: now,
        updatedAt: now,
      }).run();
      stack = { id: stackId } as any;
    }
    ...
    console.error('[scanner-import] Auto-monitor creation failed:', err);
  }
