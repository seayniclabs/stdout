...
export function syncHostMonitors(db: Database, userId: string): { created: number; updated: number } {
  let created = 0;
  let updated = 0;

  // Get all discovered hosts for this user
  const hosts = db.prepare(`
    SELECT id, ip_address, hostname, stack_id
    FROM discovered_hosts
    WHERE user_id = ?
  `).all(userId) as Array<{ id: string; ip_address: string; hostname: string | null; stack_id: string | null }>;
  ...
  for (const host of hosts) {
    ...
    const existing = db.prepare(`
      ...
      LIMIT 1
    `).get(userId, target) as { id: string } | undefined;
